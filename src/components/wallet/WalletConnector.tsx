...
export function WalletConnector() {
  const { connect, disconnect, account, wallets, connected, isLoading } =
    useWallet();
  ...
  useEffect(() => {
    if (connected && account) {
      const currentWallet = wallets.find(
        (w) => w.name === account.publicKey?.toString()
      );
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="rounded-none border-2 border-blaze-black bg-blaze-white">
        <DialogHeader>
          <DialogTitle className="font-display text-4xl font-bold text-blaze-black">
            Connect a Wallet
          </DialogTitle>
          ...
          {wallets.map((wallet: WalletInfo) => (
            ...
            >
              ...
              {wallet.name === lastConnectedWallet && (
                <span className="ml-auto text-sm text-blaze-black/60">
                  (Last used)
                </span>
              )}
            </Button>
