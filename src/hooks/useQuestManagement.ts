...
export function useQuestManagement() {
  ...
  const createQuest = useCallback(async (questData: QuestFormData) => {
    if (!account) {
  ...
  const endQuest = useCallback(async (questId: string, winner: string) => {
    ...
    try {
      const result = await contractEndQuest(questId, winner);
      ...
      console.error('Quest end failed:', error);
      return { success: false, error };
    }
  }, [account, contractEndQuest]);

  const checkQuestParticipation = useCallback(async (questId: string) => {
