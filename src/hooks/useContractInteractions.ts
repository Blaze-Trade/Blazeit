...
async function getAptosClient() {
  ...
  const config = {
    ...
    fullnode: CONTRACT_CONFIG.network === 'mainnet' 
...
export function useContractInteractions() {
  ...
  const createToken = useCallback(async (params: TokenCreationParams) => {
    const payload = createTransactionPayload(
      CONTRACT_FUNCTIONS.tokenCreation.createToken,
      [],
      [
        params.name,
        params.symbol,
  ...
  const createQuest = useCallback(async (params: QuestCreationParams) => {
    const payload = createTransactionPayload(
      CONTRACT_FUNCTIONS.questManagement.createQuest,
      [],
      [
        params.name,
        params.description,
  ...
  const joinQuest = useCallback(async (questId: string) => {
    const payload = createTransactionPayload(
      CONTRACT_FUNCTIONS.questManagement.joinQuest,
      [],
      [questId]
    );
  ...
  const endQuest = useCallback(async (questId: string, winner: string) => {
    const payload = createTransactionPayload(
      CONTRACT_FUNCTIONS.questManagement.endQuest,
      [],
      [questId, winner]
    );
  ...
  const initializePortfolio = useCallback(async (questId: string, participant: string, initialBalance: number) => {
    const payload = createTransactionPayload(
      CONTRACT_FUNCTIONS.portfolioTrading.initializePortfolio,
      [],
  ...
  const buyToken = useCallback(async (params: TokenTradeParams) => {
    const payload = createTransactionPayload(
      CONTRACT_FUNCTIONS.portfolioTrading.buyToken,
      [],
      [
        params.questId,
        params.tokenAddress,
        params.symbol,
        params.quantity,
        parseAptAmount(params.cost.toString()),
      ]
    );
  ...
  const sellToken = useCallback(async (questId: string, tokenAddress: string, quantity: number, proceeds: number) => {
    const payload = createTransactionPayload(
      CONTRACT_FUNCTIONS.portfolioTrading.sellToken,
      [],
      [
        questId,
        tokenAddress,
        quantity,
        parseAptAmount(proceeds.toString()),
      ]
    );
  ...
  const getTokenAddress = useCallback(async (symbol: string) => {
    try {
      const client = await getAptosClient();
      const payload = createTransactionPayload(
        CONTRACT_FUNCTIONS.tokenCreation.getTokenAddress,
        [],
        [symbol]
      );
  ...
  const getTokenMetadata = useCallback(async (tokenAddress: string) => {
    try {
      const client = await getAptosClient();
      const payload = createTransactionPayload(
        CONTRACT_FUNCTIONS.tokenCreation.getTokenMetadata,
        [],
        [tokenAddress]
      );
      return await client.view(payload);
  ...
  const getQuestInfo = useCallback(async (questId: string) => {
    try {
      const client = await getAptosClient();
      const payload = createTransactionPayload(
        CONTRACT_FUNCTIONS.questManagement.getQuest,
        [],
        [questId]
      );
      return await client.view(payload);
  ...
  const hasJoinedQuest = useCallback(async (questId: string, participant: string) => {
    try {
      const client = await getAptosClient();
      const payload = createTransactionPayload(
        CONTRACT_FUNCTIONS.questManagement.hasJoinedQuest,
        [],
        [questId, participant]
      );
      return await client.view(payload);
  ...
  const getPortfolioInfo = useCallback(async (questId: string, participant: string) => {
    try {
      const client = await getAptosClient();
      const payload = createTransactionPayload(
        CONTRACT_FUNCTIONS.portfolioTrading.getPortfolio,
        [],
        [questId, participant]
      );
      return await client.view(payload);
  ...
  const getTokenHolding = useCallback(async (questId: string, participant: string, tokenAddress: string) => {
    try {
      const client = await getAptosClient();
      const payload = createTransactionPayload(
        CONTRACT_FUNCTIONS.portfolioTrading.getTokenHolding,
        [],
