...
import type { Quest, LeaderboardEntry, QuestPortfolio } from '@shared/types';
...
export function QuestDetailPage() {
  ...
  const { isConnected, address, joinQuest, joinedQuests, setActiveQuest } = usePortfolioStore();
  ...
  const handleJoinQuest = async () => {
    ...
    try {
      ...
      joinQuest(quest);
      toast.success(`Successfully joined Quest: ${quest.name}`);
      navigate('/');
    } catch (error) {
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <Button variant="outline" onClick={() => navigate('/quests')} className="border-2 border-blaze-black rounded-none h-12 px-4 flex items-center gap-2 font-bold uppercase text-lg hover:bg-blaze-black/5">
        ...
        <div className="border-2 border-blaze-black bg-blaze-white p-6 md:p-8 space-y-6 shadow-blaze-shadow">
          ...
          {!isJoined && quest.status !== 'ended' && (
            <Button onClick={handleJoinQuest} disabled={!isConnected} className="w-full h-14 rounded-none border-2 border-blaze-black bg-blaze-orange text-blaze-black text-xl font-bold uppercase tracking-wider hover:bg-blaze-black hover:text-blaze-white active:translate-y-px active:translate-x-px">
              Join Quest
            </Button>
          )}
          {isJoined && quest.status === 'active' && (
            <Button onClick={handleBuildPortfolio} className="w-full h-14 rounded-none border-2 border-blaze-black bg-blaze-black text-blaze-white text-xl font-bold uppercase tracking-wider hover:bg-blaze-orange hover:text-blaze-black active:translate-y-px active:translate-x-px">
