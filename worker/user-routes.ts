import type { LeaderboardEntry, Quest, Token } from "@shared/types";
import { Hono } from "hono";
import type { Env } from "./core-utils";
import { bad, isStr, notFound, ok } from "./core-utils";
import {
  QuestEntity,
  QuestPortfolioEntity,
  TokenEntity,
  WatchlistEntity,
} from "./entities";
export function userRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  app.post("/api/quests", async (c) => {
    const questData = (await c.req.json()) as Partial<
      Quest & { startTime?: string; endTime?: string }
    >;
  ...
  app.post("/api/quests/:questId/buy", async (c) => {
    const { questId } = c.req.param();
    const { userId, token, quantity } = (await c.req.json()) as {
      userId?: string;
      token?: Token;
      quantity?: number;
    };
    if (!isStr(userId) || !token || !quantity || quantity <= 0)
      return bad(c, "userId, token, and quantity are required");
    const portfolioId = `${questId}:${userId}`;
    const portfolio = new QuestPortfolioEntity(c.env, portfolioId);
    if (!(await portfolio.exists()))
      return notFound(c, "User has not joined this quest");
  ...
  app.post("/api/quests/:questId/sell", async (c) => {
    const { questId } = c.req.param();
    const { userId, tokenId, quantity } = (await c.req.json()) as {
      userId?: string;
      tokenId?: string;
      quantity?: number;
    };
    if (!isStr(userId) || !isStr(tokenId) || !quantity || quantity <= 0)
      return bad(c, "userId, tokenId, and quantity are required");
    const portfolioId = `${questId}:${userId}`;
    const portfolio = new QuestPortfolioEntity(c.env, portfolioId);
    if (!(await portfolio.exists()))
      return notFound(c, "User has not joined this quest");
    ...
    const mockLeaderboard: LeaderboardEntry[] = [
      {
        rank: 1,
        address: "0x1a2b...c3d4",
        portfolioValue: 15230.45,
        pnlPercent: 52.3,
      },
      {
        rank: 2,
        address: "0x5e6f...a7b8",
        portfolioValue: 13105.9,
        pnlPercent: 31.06,
      },
      {
        rank: 3,
        address: "0x9c0d...e1f2",
        portfolioValue: 11800.0,
        pnlPercent: 18.0,
      },
      {
        rank: 4,
        address: "0x3g4h...i5j6",
        portfolioValue: 9500.78,
        pnlPercent: -4.99,
      },
      {
        rank: 5,
        address: "0x7k8l...m9n0",
        portfolioValue: 8200.12,
        pnlPercent: -18.0,
      },
    ];
