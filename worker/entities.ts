...
import type { Token, Quest, QuestPortfolio, Holding } from "@shared/types";
import { MOCK_TOKENS, MOCK_QUESTS } from "@shared/mock-data";
...
export class TokenEntity extends IndexedEntity<Token> {
  ...
  static readonly initialState: Token = { id: "", symbol: "", name: "", price: 0, change24h: 0, marketCap: 0, logoUrl: "" };
...
export class QuestPortfolioEntity extends IndexedEntity<QuestPortfolio> {
    ...
    async sellToken(tokenId: string, quantity: number): Promise<QuestPortfolio> {
        return this.mutate(portfolio => {
            const holdingIndex = portfolio.holdings.findIndex(h => h.id === tokenId);
            if (holdingIndex === -1) {
